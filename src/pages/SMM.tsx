...
import { BarChart3, CheckCircle2, RefreshCw, Search, ShieldCheck, Sparkles, Target, Zap } from 'lucide-react'
import { getSmmServices, type SmmService } from '../lib/smm'

function ServiceCard({ service }: { service: SmmService }) {
  return (
    <article className="layout-surface flex h-full flex-col rounded-sm p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="grid gap-3 text-sm text-[var(--layout-text-muted)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>Minimo {service.min.toLocaleString('pt-BR')} / Maximo {service.max.toLocaleString('pt-BR')}</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle2 className="h-4 w-4 text-[var(--layout-link-color)]" />
          <span>{service.refill ? 'Reposicao disponivel' : 'Sem reposicao automatica'}{service.cancel ? ' • Cancelavel' : ''}</span>
        </div>
      </div>
...
export function SMM() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const matchesQuery = !term || [item.name, item.category, item.type].some((value) => value.toLowerCase().includes(term))
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-[var(--layout-dashboard-sidebar-muted-text)]">
                Catalogo de engajamento digital com precos configurados pela plataforma.
              </p>
            </div>
            <div className="layout-surface rounded-sm p-4 text-[var(--layout-text-primary)] shadow-sm">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-bold">Pedido protegido</p>
                  <p className="mt-1 text-sm text-[var(--layout-text-muted)]">A consulta dos servicos passa pela estrutura da plataforma.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-5 grid gap-3 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)_auto] lg:items-center">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar por servico, categoria ou tipo"
            />
          </div>
