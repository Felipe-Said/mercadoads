                  ...
                  <div className="space-y-2">
                    <p className="text-ml-dark font-medium group-hover:text-ml-blue transition-colors">
                      {sale.products?.title
                        ...
                        ?? (sale.temp_email_service_name ? `Email temporario - ${sale.temp_email_service_name}` : null)
                        ?? (sale.smm_service_name ? `SMM - ${sale.smm_service_name}` : 'Pedido')}
                    </p>
                    ...
                    {sale.status === 'paid' && sale.claim_until && (
                      <div className="space-y-3">
                        ...
                        {sale.smm_service_id && (!sale.smm_deliveries || sale.smm_deliveries.length === 0) && (
                          <div className="rounded-md border border-yellow-100 bg-yellow-50 p-4 max-w-3xl">
                            <p className="text-sm font-semibold text-yellow-800">Servico em processamento</p>
                            <p className="mt-1 text-xs text-gray-600">O pagamento foi confirmado e a plataforma esta enviando o pedido para {sale.smm_service_name}.</p>
                          </div>
                        )}

                        {sale.smm_deliveries?.map((delivery) => {
                          if (delivery.status === 'failed') {
                            return (
                              <div key={`${delivery.service_name}-${sale.id}`} className="rounded-md border border-yellow-100 bg-yellow-50 p-4 max-w-3xl">
                                <p className="text-sm font-semibold text-yellow-800">Entrega em analise</p>
                                <p className="mt-1 text-xs text-gray-600">O pagamento foi confirmado, mas o pedido ainda nao foi aceito automaticamente. O suporte deve revisar este pedido.</p>
                              </div>
                            )
                          }
                          ...
                          return (
                            <div key={`${delivery.service_name}-${sale.id}`} className="rounded-md border border-blue-100 bg-blue-50 p-4 max-w-3xl">
                              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                  <p className="text-sm font-semibold text-blue-800">Pedido SMM enviado</p>
                                  <p className="mt-1 text-xs text-blue-700">Acompanhe o andamento deste servico pela sua lista de compras.</p>
                                </div>
                                {delivery.provider_order_id && (
                                  <Button
                                    type="button"
                                    onClick={() => handleCopyText(delivery.provider_order_id || '', sale.id)}
                                    className="h-9 rounded-sm bg-white border border-blue-200 text-blue-800 hover:bg-blue-100 text-xs font-semibold"
                                  >
                                    {copiedSaleId === sale.id ? 'Copiado' : 'Copiar pedido'}
                                  </Button>
                                )}
                              </div>
                              <div className="mt-3 grid gap-2 text-xs md:grid-cols-2">
                                <p><span className="font-semibold text-blue-900">Servico:</span> {delivery.service_name}</p>
                                <p><span className="font-semibold text-blue-900">Status:</span> {delivery.status}</p>
                                <p><span className="font-semibold text-blue-900">Quantidade:</span> {Number(delivery.quantity ?? sale.smm_quantity ?? 0).toLocaleString('pt-BR')}</p>
                                <p><span className="font-semibold text-blue-900">Pedido:</span> {delivery.provider_order_id || 'Aguardando confirmacao'}</p>
                              </div>
                            </div>
                          )
                        })}
